...
namespace ConvexHull
{
    public class ConvexHull
    {
        ...
        private RectData GetClosestRectange(List<XYZ> allPoint)
        {
            if (allPoint.Count < 3)
            {
            ...
            {
                return drawList(allPoint);
            }
            else
            {
                //找最小凸包
                List<XYZ> res = calcConvexHull(allPoint);
                //凸包最小外接矩形
                return drawList(res);
            }
        }
        private List<XYZ> calcConvexHull(List<XYZ> list)
        {
            List<XYZ> resPoint = new List<XYZ>();
            int minIndex = 0;
            ...
            XYZ minPoint = list[minIndex];
            resPoint.Add(list[minIndex]);
            list.RemoveAt(minIndex);
            list.Sort(
                delegate (XYZ p1, XYZ p2)
                {
                    vector baseVec;
                    baseVec.x = 1;
                    baseVec.y = 0;
            ...
            {
                var basePt = resPoint[resPoint.Count - 2];
                vector v1;
                ...
                if (v1.x * v2.y - v1.y * v2.x < 0)
                {
                    ...
                    while (true)
                    {
                        var basePt2 = resPoint[resPoint.Count - 2];
                        vector v12;
        ...
        private RectData drawList(List<XYZ> list)
        {
            ...
            var rects = GetAllNewRectDatas(list);
